# ✅ Checklist de Integración WPF - Sistema Completo de Balizas
...
#### 1. ✅ `beaconId` (string)
- **Qué es**: Identificador único de la baliza
- **Ejemplo**: `"BEACON_001"`, `"BEACON_PADDOCK_05"`
- **Uso en WPF**: Identificar qué baliza procesar
- **Implementación**:
```csharp
...
#### 2. ✅ `zone` (string)
- **Qué es**: Zona del circuito donde está la baliza
...
#### 3. ✅ `mode` (string)
- **Qué es**: Modo de operación de la baliza
- **Valores posibles**:
...
#### 7. ✅ `message` (string)
- **Qué es**: Texto a mostrar en la baliza
...
#### 8. ✅ `language` (string)
- **Qué es**: Idioma configurado para la baliza
...
#### 12. ✅ `online` (bool)
- **Qué es**: Indica si la baliza está online
...
- **Qué es**: Última vez que la baliza envió heartbeat
...
R: Las balizas no mostrarán textos personalizados, solo podrán mostrar textos por defecto.
...
R: No podrás reiniciar ni controlar las balizas remotamente desde el panel web.
